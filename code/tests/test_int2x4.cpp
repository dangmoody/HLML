/*
===========================================================================

hlml.
Copyright (c) Dan Moody 2019 - Present.

This file is part of hlml.

hlml is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.

hlml is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
GNU General Public License for more details.

You should have received a copy of the GNU General Public License
along with hlml.  If not, see <http://www.gnu.org/licenses/>.

===========================================================================
*/

// GENERATED FILE.  DO NOT EDIT.

#include "../../code/out/gen/hlml_functions_matrix.h"

#include <temper/temper.h>

TEMPER_TEST( TestAssignment_int2x4 )
{
	int2x4 mat;

	// fill single value
	mat = int2x4( 999 );
	TEMPER_EXPECT_TRUE( mat[0] == int4( 999, 0, 0, 0 ) );
	TEMPER_EXPECT_TRUE( mat[1] == int4( 0, 999, 0, 0 ) );

	// row filling
	mat = int2x4(
		int4( 0, 1, 2, 3 ),
		int4( 4, 5, 6, 7 )
	);
	TEMPER_EXPECT_TRUE( mat[0] == int4( 0, 1, 2, 3 ) );
	TEMPER_EXPECT_TRUE( mat[1] == int4( 4, 5, 6, 7 ) );

	// all values filled
	mat = int2x4(
		16, 15, 14, 13,
		12, 11, 10, 9
	);
	TEMPER_EXPECT_TRUE( mat[0] == int4( 16, 15, 14, 13 ) );
	TEMPER_EXPECT_TRUE( mat[1] == int4( 12, 11, 10, 9 ) );

	TEMPER_PASS();
}

TEMPER_TEST( TestComponentWiseArithmeticAddition_int2x4 )
{
	// scalar
	int2x4 answer = int2x4(
		7, 7, 7, 7,
		8, 8, 8, 8
	);

	int2x4 a = int2x4(
		6, 6, 6, 6,
		6, 6, 6, 6
	);
	int2x4 b = int2x4(
		1, 1, 1, 1,
		2, 2, 2, 2
	);
	int2x4 c = a + b;

	TEMPER_EXPECT_TRUE( c == answer );

	TEMPER_PASS();
}

TEMPER_TEST( TestComponentWiseArithmeticSubtraction_int2x4 )
{
	// scalar
	int2x4 answer = int2x4(
		5, 5, 5, 5,
		4, 4, 4, 4
	);

	int2x4 a = int2x4(
		6, 6, 6, 6,
		6, 6, 6, 6
	);
	int2x4 b = int2x4(
		1, 1, 1, 1,
		2, 2, 2, 2
	);
	int2x4 c = a - b;

	TEMPER_EXPECT_TRUE( c == answer );

	TEMPER_PASS();
}

TEMPER_TEST( TestComponentWiseArithmeticMultiplication_int2x4 )
{
	// scalar
	int2x4 answer = int2x4(
		6, 6, 6, 6,
		12, 12, 12, 12
	);

	int2x4 a = int2x4(
		6, 6, 6, 6,
		6, 6, 6, 6
	);
	int2x4 b = int2x4(
		1, 1, 1, 1,
		2, 2, 2, 2
	);
	int2x4 c = comp_mul( a, b );

	TEMPER_EXPECT_TRUE( c == answer );

	TEMPER_PASS();
}

TEMPER_TEST( TestComponentWiseArithmeticDivision_int2x4 )
{
	// scalar
	int2x4 answer = int2x4(
		6, 6, 6, 6,
		3, 3, 3, 3
	);

	int2x4 a = int2x4(
		6, 6, 6, 6,
		6, 6, 6, 6
	);
	int2x4 b = int2x4(
		1, 1, 1, 1,
		2, 2, 2, 2
	);
	int2x4 c = comp_div( a, b );

	TEMPER_EXPECT_TRUE( c == answer );

	TEMPER_PASS();
}

TEMPER_TEST( TestMultiplyMatrix_int2x4 )
{
	// scalar
	int2x2 answer = int2x2(
		72, 72,
		72, 72
	);

	int2x4 a = int2x4(
		6, 6, 6, 6,
		6, 6, 6, 6
	);
	int4x2 b = int4x2(
		1, 1,
		2, 2,
		3, 3,
		6, 6
	);
	int2x2 c = a * b;

	TEMPER_EXPECT_TRUE( c == answer );

	TEMPER_PASS();
}

TEMPER_TEST( TestIncrement_int2x4 )
{
	int2x4 mat;

	// prefix
	mat = int2x4(
		0, 0, 0, 0,
		0, 0, 0, 0
	);
	++mat;
	TEMPER_EXPECT_TRUE( mat == int2x4(
		1, 1, 1, 1,
		1, 1, 1, 1
	) );

	// postfix
	mat = int2x4(
		0, 0, 0, 0,
		0, 0, 0, 0
	);
	mat++;
	TEMPER_EXPECT_TRUE( mat == int2x4(
		1, 1, 1, 1,
		1, 1, 1, 1
	) );

	TEMPER_PASS();
}

TEMPER_TEST( TestDecrement_int2x4 )
{
	int2x4 mat;

	// prefix
	mat = int2x4(
		1, 1, 1, 1,
		1, 1, 1, 1
	);
	--mat;
	TEMPER_EXPECT_TRUE( mat == int2x4(
		0, 0, 0, 0,
		0, 0, 0, 0
	) );

	// postfix
	mat = int2x4(
		1, 1, 1, 1,
		1, 1, 1, 1
	);
	mat--;
	TEMPER_EXPECT_TRUE( mat == int2x4(
		0, 0, 0, 0,
		0, 0, 0, 0
	) );

	TEMPER_PASS();
}

TEMPER_TEST( TestRelational_int2x4 )
{
	bool2x4 allTrue = bool2x4(
		true, true, true, true,
		true, true, true, true
	);

	int2x4 mat0 = int2x4(
		1, 1, 1, 1,
		1, 1, 1, 1
	);
	int2x4 mat1 = int2x4(
		2, 2, 2, 2,
		2, 2, 2, 2
	);
	int2x4 mat2 = int2x4(
		3, 3, 3, 3,
		3, 3, 3, 3
	);
	int2x4 mat3 = int2x4(
		4, 4, 4, 4,
		4, 4, 4, 4
	);

	bool2x4 test0  = mat0 <= mat0;
	bool2x4 test1  = mat0 >= mat0;
	bool2x4 test2  = mat0 <  mat1;
	bool2x4 test3  = mat0 <  mat2;
	bool2x4 test4  = mat0 <  mat3;

	bool2x4 test5  = mat1 >  mat0;
	bool2x4 test6  = mat1 <= mat1;
	bool2x4 test7  = mat1 >= mat1;
	bool2x4 test8  = mat1 <  mat2;
	bool2x4 test9  = mat1 <  mat3;

	bool2x4 test10 = mat2 >  mat0;
	bool2x4 test11 = mat2 >  mat1;
	bool2x4 test12 = mat2 <= mat2;
	bool2x4 test13 = mat2 >= mat2;
	bool2x4 test14 = mat2 <  mat3;

	bool2x4 test15 = mat3 >  mat0;
	bool2x4 test16 = mat3 >  mat1;
	bool2x4 test17 = mat3 >  mat2;
	bool2x4 test18 = mat3 <= mat3;
	bool2x4 test19 = mat3 >= mat3;

	TEMPER_EXPECT_TRUE( test0  == allTrue );
	TEMPER_EXPECT_TRUE( test1  == allTrue );
	TEMPER_EXPECT_TRUE( test2  == allTrue );
	TEMPER_EXPECT_TRUE( test3  == allTrue );
	TEMPER_EXPECT_TRUE( test4  == allTrue );

	TEMPER_EXPECT_TRUE( test5  == allTrue );
	TEMPER_EXPECT_TRUE( test6  == allTrue );
	TEMPER_EXPECT_TRUE( test7  == allTrue );
	TEMPER_EXPECT_TRUE( test8  == allTrue );
	TEMPER_EXPECT_TRUE( test9  == allTrue );

	TEMPER_EXPECT_TRUE( test10 == allTrue );
	TEMPER_EXPECT_TRUE( test11 == allTrue );
	TEMPER_EXPECT_TRUE( test12 == allTrue );
	TEMPER_EXPECT_TRUE( test13 == allTrue );
	TEMPER_EXPECT_TRUE( test14 == allTrue );

	TEMPER_EXPECT_TRUE( test15 == allTrue );
	TEMPER_EXPECT_TRUE( test16 == allTrue );
	TEMPER_EXPECT_TRUE( test17 == allTrue );
	TEMPER_EXPECT_TRUE( test18 == allTrue );
	TEMPER_EXPECT_TRUE( test19 == allTrue );

	TEMPER_PASS();
}

TEMPER_TEST( TestArray_int2x4 )
{
	int2x4 mat;

	TEMPER_EXPECT_TRUE( mat[0] == int4( 1, 0, 0, 0 ) );
	TEMPER_EXPECT_TRUE( mat[1] == int4( 0, 1, 0, 0 ) );

	TEMPER_PASS();
}

TEMPER_TEST( TestBitwise_And_int2x4 )
{
	int2x4 a  = int2x4(
		21, 21, 21, 21,
		21, 21, 21, 21
	);
	int2x4 b  = int2x4(
		7, 7, 7, 7,
		7, 7, 7, 7
	);

	int2x4 answer = a & b;

	TEMPER_EXPECT_TRUE( answer == int2x4(
		5, 5, 5, 5,
		5, 5, 5, 5
	) );

	TEMPER_PASS();
}

TEMPER_TEST( TestBitwise_Or_int2x4 )
{
	int2x4 a  = int2x4(
		21, 21, 21, 21,
		21, 21, 21, 21
	);
	int2x4 b  = int2x4(
		7, 7, 7, 7,
		7, 7, 7, 7
	);

	int2x4 answer = a | b;

	TEMPER_EXPECT_TRUE( answer == int2x4(
		23, 23, 23, 23,
		23, 23, 23, 23
	) );

	TEMPER_PASS();
}

TEMPER_TEST( TestBitwise_Xor_int2x4 )
{
	int2x4 a  = int2x4(
		21, 21, 21, 21,
		21, 21, 21, 21
	);
	int2x4 b  = int2x4(
		7, 7, 7, 7,
		7, 7, 7, 7
	);

	int2x4 answer = a ^ b;

	TEMPER_EXPECT_TRUE( answer == int2x4(
		18, 18, 18, 18,
		18, 18, 18, 18
	) );

	TEMPER_PASS();
}

TEMPER_TEST( TestBitwise_ShiftLeft_int2x4 )
{
	int2x4 a  = int2x4(
		1, 1, 1, 1,
		1, 1, 1, 1
	);
	int2x4 b  = int2x4(
		2, 2, 2, 2,
		2, 2, 2, 2
	);

	int2x4 answer = a << b;

	TEMPER_EXPECT_TRUE( answer == int2x4(
		4, 4, 4, 4,
		4, 4, 4, 4
	) );

	TEMPER_PASS();
}

TEMPER_TEST( TestBitwise_ShiftRight_int2x4 )
{
	int2x4 a  = int2x4(
		16, 16, 16, 16,
		16, 16, 16, 16
	);
	int2x4 b  = int2x4(
		4, 4, 4, 4,
		4, 4, 4, 4
	);

	int2x4 answer = a >> b;

	TEMPER_EXPECT_TRUE( answer == int2x4(
		1, 1, 1, 1,
		1, 1, 1, 1
	) );

	TEMPER_PASS();
}

TEMPER_TEST( TestBitwise_Unary_int2x4 )
{
	int2x4 a = int2x4(
		0, 0, 0, 0,
		0, 0, 0, 0
	);

	int2x4 answer = ~a;

	TEMPER_EXPECT_TRUE( answer == int2x4(
		(int32_t) -1, (int32_t) -1, (int32_t) -1, (int32_t) -1,
		(int32_t) -1, (int32_t) -1, (int32_t) -1, (int32_t) -1
	) );

	TEMPER_PASS();
}

TEMPER_TEST( TestIdentity_int2x4 )
{
	// scalar
	int2x4 id = int2x4(
		1, 0, 0, 0,
		0, 1, 0, 0
	);

	int2x4 mat;
	TEMPER_EXPECT_TRUE( mat == id );

	identity( mat );
	TEMPER_EXPECT_TRUE( mat == id );

	TEMPER_PASS();
}

TEMPER_TEST( TestTranspose_int2x4 )
{
	// scalar
	int2x4 mat = int2x4(
		0, 1, 2, 3,
		4, 5, 6, 7
	);
	int4x2 trans = transpose( mat );

	TEMPER_EXPECT_TRUE( trans == int4x2(
		0, 4,
		1, 5,
		2, 6,
		3, 7
	) );

	TEMPER_PASS();
}

TEMPER_TEST( TestScale_int2x4 )
{
	int2x4 mat;
	int2x4 scaled = scale( mat, int3( 2, 2, 2 ) );

	TEMPER_EXPECT_TRUE( scaled == int2x4(
		2, 0, 0, 0,
		0, 2, 0, 0
	) );

	TEMPER_PASS();
}

TEMPER_SUITE( Test_int2x4 )
{
	TEMPER_RUN_TEST( TestAssignment_int2x4 );
	TEMPER_RUN_TEST( TestComponentWiseArithmeticAddition_int2x4 );
	TEMPER_RUN_TEST( TestComponentWiseArithmeticSubtraction_int2x4 );
	TEMPER_RUN_TEST( TestComponentWiseArithmeticMultiplication_int2x4 );
	TEMPER_RUN_TEST( TestComponentWiseArithmeticDivision_int2x4 );
	TEMPER_RUN_TEST( TestMultiplyMatrix_int2x4 );
	TEMPER_RUN_TEST( TestIncrement_int2x4 );
	TEMPER_RUN_TEST( TestDecrement_int2x4 );
	TEMPER_RUN_TEST( TestRelational_int2x4 );
	TEMPER_RUN_TEST( TestArray_int2x4 );
	TEMPER_RUN_TEST( TestBitwise_And_int2x4 );
	TEMPER_RUN_TEST( TestBitwise_Or_int2x4 );
	TEMPER_RUN_TEST( TestBitwise_Xor_int2x4 );
	TEMPER_RUN_TEST( TestBitwise_ShiftLeft_int2x4 );
	TEMPER_RUN_TEST( TestBitwise_ShiftRight_int2x4 );
	TEMPER_RUN_TEST( TestBitwise_Unary_int2x4 );
	TEMPER_RUN_TEST( TestIdentity_int2x4 );
	TEMPER_RUN_TEST( TestTranspose_int2x4 );
	TEMPER_RUN_TEST( TestScale_int2x4 );
}
