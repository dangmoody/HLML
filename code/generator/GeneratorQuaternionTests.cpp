...
void GeneratorQuaternionTests::GenerateTestMultiplyScalar() {
...
void GeneratorQuaternionTests::GenerateTestLength() {
	if (Gen_TypeIsFloatingPoint( m_type ) == false) {
		return;
	}
	...
	Gen_GetNumericLiteral( m_type, 4, fourStr, 1 );
	Gen_GetNumericLiteral( m_type, 5, fiveStr, 1 );
	Gen_GetNumericLiteral( m_type, 7.34846922834f, answerStr, 11 );
	...
	char testName[GEN_STRING_LENGTH_TEST_NAME] = { 0 };
	snprintf( testName, GEN_STRING_LENGTH_TEST_NAME, "TestArithmetic%s_%s", "Length", m_fullTypeName );
...
void GeneratorQuaternionTests::GenerateTestConjugate() {
	if ( Gen_TypeIsFloatingPoint( m_type ) == false ) {
		return;
	}
	...
	Gen_GetNumericLiteral( m_type, 4, fourStr, 1 );
	Gen_GetNumericLiteral( m_type, 5, fiveStr, 1 );
	Gen_GetNumericLiteral( m_type, -2, answerXStr, 6 );
	Gen_GetNumericLiteral( m_type, -3, answerYStr, 6 );
	Gen_GetNumericLiteral( m_type, -4, answerZStr, 6 );
	Gen_GetNumericLiteral( m_type, 5, answerWStr, 6 );
	...
	char testName[GEN_STRING_LENGTH_TEST_NAME] = { 0 };
	snprintf( testName, GEN_STRING_LENGTH_TEST_NAME, "TestArithmetic%s_%s", "Conjugate", m_fullTypeName );

	String_Appendf( &m_codeTests, "TEMPER_TEST( %s )\n", testName );
	String_Append( &m_codeTests, "{\n" );
	String_Appendf( &m_codeTests, "\tconst %s a = %s( %s, %s, %s, %s );\n", m_fullTypeName, m_fullTypeName, parmListRhsValues[0], parmListRhsValues[1], parmListRhsValues[2], parmListRhsValues[3] );
	String_Append( &m_codeTests, "\n" );
	String_Appendf( &m_codeTests, "\t%s b = quaternion_conjugate( a );\n", m_fullTypeName );
	String_Append( &m_codeTests, "\n" );
	String_Appendf( &m_codeTests, "\tTEMPER_EXPECT_TRUE( b == %s( %s, %s, %s, %s ) );\n", m_fullTypeName, parmListAnswers[0], parmListAnswers[1], parmListAnswers[2], parmListAnswers[3] );
	String_Append( &m_codeTests, "\n" );
	String_Append( &m_codeTests, "\tTEMPER_PASS();\n" );
...
void GeneratorQuaternionTests::GenerateTestInverse() {
	if ( Gen_TypeIsFloatingPoint( m_type ) == false ) {
		return;
	}
	...
	Gen_GetNumericLiteral( m_type, 4, fourStr, 1 );
	Gen_GetNumericLiteral( m_type, 5, fiveStr, 1 );
	Gen_GetNumericLiteral( m_type, -0.037037f, answerXStr, 6 );
	Gen_GetNumericLiteral( m_type, -0.0555556f, answerYStr, 7 );
	Gen_GetNumericLiteral( m_type, -0.0740741f, answerZStr, 7 );
	Gen_GetNumericLiteral( m_type, 0.0925926f, answerWStr, 6 );
	...
	char testName[GEN_STRING_LENGTH_TEST_NAME] = { 0 };
	snprintf( testName, GEN_STRING_LENGTH_TEST_NAME, "TestArithmetic%s_%s", "Inverse", m_fullTypeName );
